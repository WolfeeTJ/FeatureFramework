...
import CreditLife_FeatureFramework.ff_main_offline_analysis_rolling as ff
import pandas as pd
import json
...
for i in range(0, len(configfile)):
    ...
    dic_result_tmp, df_result_tmp = ff.ff_main_offline_analysis_rolling(datasource, key_col, month_col, month_start, month_end, in_where)
    print("文件： " + datasource)
    print(df_result_tmp)
    dic_new = dict()
